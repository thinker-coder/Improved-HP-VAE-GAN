...
class FeatureExtractor(nn.Sequential):
    def __init__(self, in_channel, out_channel, ker_size, padding, stride, num_blocks=1, return_linear=False):
...
class FeatureExtractor2(nn.Sequential):
    def __init__(self, out_channel, ker_size, padding, stride, num_blocks=1, return_linear=False):
        super(FeatureExtractor2, self).__init__()
        self.add_module('conv_block_0', ConvBlock2DSN(out_channel, out_channel, ker_size, padding, stride)),
...
class Encode2DVAE(nn.Module):
    def __init__(self, opt, out_dim=None, num_blocks=1):
        ...
        self.features2 = FeatureExtractor2(opt.nfc, opt.ker_size, opt.ker_size // 2, 1, num_blocks=num_blocks)
        ...
        self.vae_attention = attention_blocks[1](64)

    def forward(self, x):

        features = self.features(x)
        features = self.vae_attention(features) #注意力
